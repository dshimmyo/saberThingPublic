Shader "Shimmy/saberCutLWRP" 
{
   ...
   SubShader 
   {
        Tags {
            ...
            //"Lightmode"="ForwardBase" //breaks the shader
        }
        ZWrite On
        ...
        //Tags{"LightMode" = "ForwardBase"} //Shader error in 'Shimmy/saberCut': invalid subscript 'instanceID' 'UnitySetupInstanceID': no matching 1 parameter function at line 549 (on gles3)
        ...
        CGPROGRAM
            ...
            #include "UnityLightingCommon.cginc"//needed to calculate diffuse lighting
            ...
            float3 get_cheap_color_gradHack(float signal)
            {
                float3 outColor = float3(0, 0, 0);
            ...
            v2f vert (appdata v)//void disp (inout appdata_full v)//, out Input o //just hacking trying to fix a weird artifact
            {
                v2f o;
                ...
                //diffuse lighting calculated here, this does not include accepting shadows
                half3 worldNormal = UnityObjectToWorldNormal(v.normal);
                half nl = 0;
                if (_WorldSpaceLightPos0.w == 0)//is directional light
                {
                    nl = max(nl, dot(worldNormal, _WorldSpaceLightPos0.xyz));//lighting common file provides light info
                }
                else if (_WorldSpaceLightPos0.w == 1)//other lights, position
                {
                    nl = max(nl, dot(worldNormal, normalize(v.vertex - _WorldSpaceLightPos0.xyz)));//lighting common file provides light info
                }
                //nl = max(nl, dot(worldNormal, _WorldSpaceLightPos1.xyz));//this doesn't work
                o.diff = nl * _LightColor0;//also in the include file

                TRANSFER_SHADOW(o);//receive shadows
            ...
            float3 e = float3(0,0,0);//max(0,min(1,myUv2.x)) * get_cheap_color_gradBaked(myUv2.x );//float3(myUv2.x,myUv2.x,myUv2.x);
             if (_HeatColorLookupToggle==0) 
             {
                 e = myUv2.x * float3(1,.25,0);//float3(heatRatio * heatRatio * 7 * get_cheap_color_gradBaked(heatRatio));//changed half to float to fix emission artifact!
                 if (e.r > 1 || e.g > 1 || e.b > 1) { e = float3(1,1,1);}
             } 
             else 
             {
                fixed2 hcl_uv = 1;// (0.0, heatRatio);
                hcl_uv.x *= saturate(myUv2.x);//(heatRatio);
                fixed4 hcl = tex2D(_HeatColorLookup, hcl_uv);
                e = float3(hcl.xyz);
             }
           ...
           //          }
            ...
            //          o.Albedo = c.rgb; 
                ...
                return c * IN.diff * shadow + em; //I still don't understand how to add emission to a vert/frag shader
   ...
   //      }//surf
    ...
    Pass //cast shadow on other objects
    {
